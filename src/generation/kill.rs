...
impl Generation {
    ...
    pub fn kill(&mut self) -> () {
        ...
        for (index, _gene) in self.species[0].creatures.clone().iter().enumerate() {
            if !rand::thread_rng()
                .gen_bool(index as f64 / (self.species[0].creatures.len() - 1) as f64)
            {
                indexes.push(index);
            }
        }
        for index in indexes.iter().rev() {
            self.species[0].creatures.remove(*index);
        }
